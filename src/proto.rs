...
fn exec_class_query(db: &sled::Db, msg: ClassQueryArgs) -> Result<String> {
    eprintln!("ClassQuery(index_name: {}, class_name: {})", msg.index_name, msg.class_name);
...
fn exec_reindex_classpath_cmd(db: &sled::Db, msg: ReindexArgs) -> Result<String> {
    eprintln!("ReindexClasspathCmd(index_name: {}, archive_source: {})", msg.index_name, msg.archive_source);
...
fn exec_reindex_path_cmd(db: &sled::Db, msg: ReindexArgs) -> Result<String> {
    eprintln!("ReindexPathCmd(index_name: {}, archive_source: {})", msg.index_name, msg.archive_source);
    ...
    for msg in msgs.into_iter::<ClientMsg>() {
        match msg {
            Ok(msg1) => {
                ...
                match encoded_resp {
                    Ok(s) => { write_stream.write(s.as_bytes()); },
